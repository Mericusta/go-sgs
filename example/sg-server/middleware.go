...
func (m *UserMiddleware) Do(ctx dispatcher.IContext, e *event.Event) bool {
	if handler, has := userHandlerMgr[e.ID()]; handler != nil && has {
		...
		if !has {
			logger.Log().Error("can not find user by link", zap.Uint64("linker", ctx.Linker().UID()))
			return false
		}
		user, ok := iUser.(*model.User)
		if !ok {
			logger.Log().Error("server user manager link value type is not *User", zap.Uint64("linker", ctx.Linker().UID()))
			return false
		}
